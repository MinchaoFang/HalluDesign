...
def run_RFD_partial(file_path: str, output_dir: str,ref_time_steps: str,) -> bool:
    ...
    try:
        ...
        command = [
            "/home/caolongxingLab/caolongxing/anaconda3/envs/SE3nv/bin/python",
            "/storage/caolongxingLab/fangminchao/tools/RFdiffusion/scripts/run_inference.py",
            f"inference.output_prefix={os.path.join(output_dir, file_name)}",  
            f"inference.input_pdb={file_path}", 
            f"contigmap.contigs=[{length}-{length}]",  
            "inference.num_designs=1",
            f"diffuser.partial_T={ref_time_steps}"
        ]
...
def process_single_pdb(pdb_file: str, 
    ...
    try:
        ...
        clear_gpu_memory()
        success = run_RFD_partial(copied_file, target_dir, ref_time_steps=str(ref_time_steps))
        
        if success:
            metrics['RFD_Status'] = 'success'
            file_name = os.path.splitext(os.path.basename(copied_file))[0]
            pdb_output = os.path.join(target_dir, file_name)+"_0.pdb"
        ...
        for cycle in range(args.num_recycles):
            ...
            try:
                ...
                metrics, next_input = process_single_pdb(
                    ...
                    args.reconstruct,
                    args.fake_msa,
                    run_af3=not is_last_cycle  # last cycle no runding af3
                )
