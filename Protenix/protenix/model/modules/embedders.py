...
class RelativePositionEncoding(nn.Module):
    ...
    def __init__(self, r_max: int = 32, s_max: int = 2, c_z: int = 128 ,cyclic: int = 0) -> None:
        """
        Args:
    ...
    def forward(self, input_feature_dict: dict[str, Any]) -> torch.Tensor:
        ...
        if self.cyclic:
            #print("using cyclic pos encoding")
            chain_a_mask = (asym_id == 0) # [..., N_tokens]
            # Create a 2D mask that marks residue pairs within chain A.
            valid_mask_a_pair = chain_a_mask[..., :, None] & chain_a_mask[..., None, :] # [..., N_token, N_token]
            # Get the mask for chain B (or any other chain).
            chain_b_mask = (asym_id != 1) # [..., N_tokens]
            valid_mask_b_pair = chain_b_mask[..., :, None] & chain_b_mask[..., None, :] # [..., N_token, N_token]
            relative_residue_diff = residue_index[..., :, None] - residue_index[..., None, :] # [..., N_token, N_token]

            d_residue_cyclic_a = relative_residue_diff.clone() 
            # Compute the length of chain A.
            # Note: residue_index usually starts from 1, while padding is 0.
            # The expression jnp.max(jnp.where(chain_a_mask, residue_index, 1)) in JAX
            # translates to PyTorch as finding the maximum residue_index within chain A,
            # which gives its length (assuming indices start from 1).
            # To ensure compatibility with the batch dimension, use torch.max(..., dim=-1, keepdim=True).
            # Assume residue_index == 0 indicates padding.

            chain_length_a = torch.max(
                torch.where(chain_a_mask, residue_index, torch.zeros_like(residue_index)),
                dim=-1,
                keepdim=True
            ).values # [..., 1]
            # The value of chain_length_a should reflect the actual chain length in your data.
            # If residue_index starts from 1, then max(residue_index) represents the chain length.

            # Expand chain_length_a to match the shape of relative_residue_diff for broadcasting.
            # Its shape needs to be expanded from [..., 1] to [..., N_token, N_token].

            chain_length_a_expanded = chain_length_a.unsqueeze(-1).unsqueeze(-1) # [..., 1, 1] 

            # Apply the cyclic logic (within chain A only).
            half_chain_length = chain_length_a_expanded // 2

            # If the forward distance exceeds half of the chain length, take the reverse path instead.
            d_residue_cyclic_a = torch.where(
                relative_residue_diff > half_chain_length,
                relative_residue_diff - chain_length_a_expanded,
                d_residue_cyclic_a
            )
            # If the reverse distance exceeds half of the chain length, take the forward path instead.
            d_residue_cyclic_a = torch.where(
                relative_residue_diff < -half_chain_length,
                relative_residue_diff + chain_length_a_expanded,
                d_residue_cyclic_a
            )
            # Ensure that this cyclic logic is applied only within chain A.
            d_residue_cyclic_a = torch.where(valid_mask_a_pair, d_residue_cyclic_a, relative_residue_diff)
            #print(d_residue_cyclic_a)
            d_residue_a = torch.clip(
                input=d_residue_cyclic_a + self.r_max,
                min=0,
                max=2 * self.r_max,
            )
            # Compute the general d_residue (without applying cyclic logic, but including cross-chain markings).
            d_residue_general = torch.clip(
                input=relative_residue_diff + self.r_max,
                min=0,
                max=2 * self.r_max,
            ...
            # Combine the cyclic and non-cyclic d_residue values.
            # Use d_residue_a (which may include cyclic logic) for residue pairs within chain A.
            # Otherwise, use d_residue_general.
            d_residue = torch.where(valid_mask_a_pair, d_residue_a, d_residue_general)

            a_rel_pos = F.one_hot(d_residue, 2 * (self.r_max + 1)).long().squeeze(0) 
        else:
        #print(a_rel_pos.shape)
